    ...
    <section className='bg-white'>
          <div className='w-11/12 lg:w-9/12 mx-auto py-5 pb-10 bg-white'>
              <div className='relative'>
                ...
                <h1 className='text-base md:text-xl font-RedHatDisplayBold tracking-widest text-center my-10 w-fit mx-auto bg-white px-5 capitalize'>browse jobs by specialisms</h1>
              </div>
              <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-1 gap-y-3 lg:gap-x-5 lg:gap-y-5'>
                {CategoriesData?.map((data:any,index:any)=>{
                  return(
                    <Link to='/jobs'>
                      <div key={index} className='flex justify-between items-center px-5 lg:px-10 py-3 rounded-full border shadow-sm  hover:shadow-xl group duration-300 cursor-pointer'>
                          <div className='flex flex-col gap-2'>
                          <h2 className='xs:text-xs sm:text-sm md:text-base capitalize font-medium text-slate-700'>{data.title}</h2>
